...
public class ExternalTaskClientBuilderImpl implements ExternalTaskClientBuilder {
    ...
    protected void initRequestExecutor() {
        ...
        requestExecutor = new RequestExecutor(requestInterceptorHandler, objectMapper);
    }

    protected void initCurrentContext() {
        Context.initContext(
            new ExternalTaskServiceImpl(engineClient),
            new MessageServiceImpl(baseUrl, requestExecutor));
        RequestInterceptorHandler requestInterceptorHandler = new RequestInterceptorHandler(interceptors);
        requestExecutor = new RequestExecutor(requestInterceptorHandler, objectMapper);
    }
}
